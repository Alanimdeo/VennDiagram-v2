...
exports.default = new types_1.Command(new discord_js_1.SlashCommandBuilder()
    ...
    if (result.loadType === "error") {
        const embed = new discord_js_1.EmbedBuilder()
            .setColor("#FF0000")
            .setTitle(":warning: 오류가 발생했어요")
            .setDescription(result.error || "알 수 없는 오류");
    ...
    if (result.loadType === "playlist") {
        return await interaction.editReply("재생목록은 지원하지 않아요. 개별 영상의 URL을 입력해 주세요.");
    }
    let track;
    if (result.loadType === "search") {
        try {
            track = await (0, search_1.makeChoice)(result.tracks, interaction);
        }
        catch (err) {
            let message = "";
        ...
        if (/(\?|&)t=[0-9]+(s)?/.test(keyword) &&
            ...
            if (choice === "2") {
                track.setPosition(Number(/(\?|&)t=([0-9]+)(s)?/.exec(keyword)[2]) * 1000);
            }
            else if (choice !== "1") {
    ...
    if (!player) {
        return await interaction.editReply("플레이어를 생성할 수 없어요.");
    }
    let wrapper = bot.players.get(interaction.guildId);
    if (!wrapper) {
        wrapper = new player_1.PlayerWrapper(bot, player);
        bot.players.set(interaction.guildId, wrapper);
    }
